...
測定リポジトリ: **[`VisionNurture-com/spring-boot-modern-examples`](https://github.com/VisionNurture-com/spring-boot-modern-examples)**（現在の測定先）
> 🔴 **本節の 3 ラウンドは、それ以前に使っていた測定専用リポジトリ `orcus-tbpd/native-ci-measure` で取ったものです。**
> 同じアプリを公開サンプルで測り直した結果は §公開サンプルで測り直した にあります。
...
## 公開サンプルで測り直した（2026-08-30・8 ラウンド）

上の値は測定専用のリポジトリで 3 ラウンド測ったものです。そのリポジトリを畳むにあたり、
**同じアプリを本リポジトリの `app` モジュールとして 8 ラウンド測り直しました**（生データは
[`../../results/005-native-ci/remeasure-20260830/`](../../results/005-native-ci/remeasure-20260830/)）。

### 構造的な値は完全に一致した

| 項目 | 3 ラウンドの値 | 8 ラウンドの値 |
|---|---:|---:|
| ネイティブバイナリ | 97,192,200 | **97,192,200**（8/8 同一）|
| 到達可能な型 | 20,120 | **20,120**（8/8 同一）|
| 落ちるアーム | 落ちる | **落ちる**（3/3・`OutOfMemoryError` 1 行・バイナリ生成なし）|

jar だけは **21,984,304 → 21,983,846 バイト**（458 バイト差）になります。アプリのコードは同じで、
`META-INF/maven` に埋め込まれる pom（artifactId と親）が違うためです。

### 時間の値は機体ごとに動く

| アーム | 8 ラウンドの実測（秒・昇順）| 中央値 |
|---|---|---:|
| 対照 JVM | 12.4 / 14.5 / 15.2 / 16.7 / 17.4 / 18.6 / 20.9 / 23.5 | 17.1 |
| Native キャッシュなし | 195.3 / 200.0 / 207.7 / 210.2 / 211.2 / 213.8 / 216.3 / 238.2 | 210.7 |
| Native キャッシュあり | 160.2 / 162.1 / 172.5 / 173.9 / 195.8 / 205.1 / 212.9 / 215.2 | 184.8 |

🔴 **キャッシュの差は、ラウンドを増やしても確定しません。**中央値の差は 3 ラウンドの **−1.09%** に対して
8 ラウンドでは **−12.29%** ですが、**分布は重なったままです**——キャッシュありの 8 本のうち 4 本
（195.8 / 205.1 / 212.9 / 215.2 秒）が、キャッシュなしの最小値 195.3 秒より遅い側にあります。
Native / JVM 比も 9.552 倍から **12.35 倍**へ動きました。

**この 2 つの数字は、どちらも「その機体でその回に出た値」です。**記事が言えるのは
「差を読む前に自分の環境の振れを測る」までで、キャッシュが効く / 効かないの結論ではありません。
