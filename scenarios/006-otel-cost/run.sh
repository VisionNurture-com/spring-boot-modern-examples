...
#   2) 率 0.0 のアームを置く。記録しないが計装は入っている状態を分離するため。
#      率を下げれば比例して軽くなるのか、判定と伝播のコストが残るのかは、このアームでしか分からない。
...
#   5) ラウンドロビン。1 ラウンドで全アームを回し、機体の状態をアームへ均等に乗せる。
#   6) メトリクスのエクスポートは全アームで無効にする。既定の step が 1 分で、
#      短い負荷では送信が数回しか起きず、アームの比較に乗らないためである。
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    # アームを名前の前方一致で絞る（例: --arms A0,A3,A5）。空なら全アーム。
...
# ------------------------------------------------------------ measurement
# アームの spec を引数で受ける（bash 3.2 でも動くよう nameref を使わない）
measure_arms() {
  local arms=( "$@" )
  if [ -n "$ARMS_FILTER" ]; then
    ...
    arms=( "${filtered[@]}" )
    log "アームを絞りました（--arms ${ARMS_FILTER}）: ${#arms[@]} 本"
  fi
...
# 捨てられているなら、そのアームは「送る費用」ではなく「捨てる費用」を測っていることになる。
phase_queue() {
