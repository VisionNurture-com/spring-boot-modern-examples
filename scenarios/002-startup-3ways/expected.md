...
| セット数 | **7 セット**・ラウンドロビン（1 セットで全腕を隣り合わせる）・**中央値**で比較 |
| 捨て走り | **1 セット**（記録しない） |
...
2. **ブロック実行ではなくラウンドロビン**です。「A を N 回 → B を N 回」だと、機体の熱や背景負荷が特定の腕にだけ乗ります。
3. **捨て走りを 1 セット行い記録しません。**初回のページキャッシュ効果を特定の腕へ乗せないためです。
4. **展開だけの腕（`jvmx`）を置きます。**展開の寄与とキャッシュの寄与を分けるためです。
